...
def slider():
    ...
    text='Yazma Hızı Arttırıcıya Hosgeldiniz'
    if count>= len(text):
        count =0
        sliderwords =''
...
def time():
    ...
    if timer>0:
        ...
        gameinstruction.configure(text='Dogru = {} | Yanlis = {} | Toplam Skor = {}'
                                  .format(score,miss,score-miss))
        rr= messagebox.askretrycancel('Bildirim','Tekrar oynamak ister misin!!!!')
        if rr==True:
            score=0
            miss=0
            timer=60
...
root.title('Yazı Hızı Arttırıcı')
...
startlabel=Label(root,text='Hadi başlayalım!!!',font=('airal',30,
...
scorelabel=Label(root,text='Skorun:',font=('arial',25,
...
timerlabel=Label(root,text='Kalan Sure:',font=('arial',25,
                'italic bold'),bg='black',fg='red')
...
gameinstruction= Label(root,text='Ekrandaki kelimeyi yaz ve entera bas',
